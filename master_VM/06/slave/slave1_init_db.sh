...
DB_FILE=${1:-slave1.db}
CSV_FILE=${2:-../data/slave1_sensors.csv}
NODE_NAME="slave1"
NODE_ROLE="SLAVE"
NODE_DESCRIPTION="First slave node responsible for local sensor data"
...
if [ ! -f "$CSV_FILE" ]; then
    ...
    echo "Usage: ./slave1_init_db.sh <database_file> <csv_file>"
    exit 1
fi
